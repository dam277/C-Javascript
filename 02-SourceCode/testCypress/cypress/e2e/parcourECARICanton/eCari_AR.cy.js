describe('Début du parcours des eCari en production', () => {
    ...
    describe('Vérifier le canton AR', () => {
          var donneesCantonAR
          beforeEach(() => {
              cy.fixture('donneesCantonAR').then((donnees) => {
                  donneesCantonAR = donnees;
              })
          })

        it('Vérifier que eAuction dans le canton AR repond', () => {
            var eAuction = donneesCantonAR.eAuction
            cy.visit(eAuction.url)

            cy.contains("un texte quelquonque")

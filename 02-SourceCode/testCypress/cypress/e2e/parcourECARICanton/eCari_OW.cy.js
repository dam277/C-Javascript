describe('Vérifier le canton OW', () =>{
  var donneesCantonOW
  beforeEach(() => {
      cy.fixture('donneesCantonOW').then((donnees) => {
          donneesCantonOW = donnees;
      })
  })

  it('Vérifer eAuction up', () =>{
    var eAuction = donneesCantonOW.eAuction
    cy.visit(eAuction.url)

    cy.get('head meta[name="version"]').should('have.attr', 'content', donneesCantonOW.version)
  ...
  it('Vérifier eMedco up', () =>{
    ...
    var eMedco = donneesCantonOW.eMedco
    cy.visit(eMedco.url)
...
/*
  it('Vérifier si eDispo up', () => {
      var eDispo = donneesCantonOW.eDispo
      cy.visit(eDispo.url)
